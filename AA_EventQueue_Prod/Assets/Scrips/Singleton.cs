...
public class Singleton<T> : MonoBehaviour where T : Component
{
    ...
    public static T Instance
    {
        get
        {
            if(m_Instance == null)
            {
                //Making sure that there's not other sintances
                // of the same type in memory.
                m_Instance = FindObjectOfType<T>();

                if(m_Instance == null)
                {
                    // making sure that htere's not other instances
                    // of the same type in memory
    ...
    public virtual void Awake()
    {
        if(m_Instance == null)
        {
            //if null, this instance is now the SSingleton instance
            // of the assigned type

            m_Instance = this as T;

            //Making sure that my Singleton instance
            // will persist in memory scross every scene
